...
def compute_telemetry(current_pilots, tasks, start_dt, end_dt):
    ...
    # Identificar tarefas-mãe (que têm subtarefas neste sprint) para não contar pontos duplicados
    ...
    for t in tasks:
        # Pular tarefas-mãe para evitar contagem dupla (subtarefas já carregam os pontos)
        if t.get('id') in parent_ids:
            continue
        # Pular tarefas genéricas (custom_item_id=0, tipo 'Tarefa' no ClickUp)
        if t.get('custom_item_id') == 0:
            continue
